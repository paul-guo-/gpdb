...
-- NOTE: the select query will fail since the gang has been terminated by the
-- dtx recovery process during standby promote. We do not test the result of
-- the select query; just expect it fail so that the next mpp query could
-- recreate the gang and succeed.
